...
export const CouponWorkoutManagement: React.FC<CouponWorkoutManagementProps> = ({ onBack }) => {
  ...
  const [couponFilter, setCouponFilter] = useState('bruno10');
  ...
  const saveWorkout = async () => {
    ...
    try {
      ...
      alert('Treino de cupom atualizado!');
    } catch (error: any) {
    ...
    <div className="bg-background-light dark:bg-background-dark text-slate-900 dark:text-white min-h-screen font-display">
      ...
      <main className="max-w-xl mx-auto pb-10">
        ...
        <section className="px-4 pt-10">
          <div className="flex items-center justify-between mb-4 gap-4">
            ...
            <div className="flex items-center gap-2 overflow-hidden">
              ...
              <span className="bg-primary/10 text-primary text-[10px] font-bold px-2 py-1 rounded-full border border-primary/20 whitespace-nowrap uppercase tracking-widest font-black leading-none">{workoutExercises.length} TOTAIS</span>
            </div>
          </div>
        ...
        <section className="px-4 py-8">
          ...
          <p className="text-center text-gray-500 text-xs mt-4">As alterações serão aplicadas instantaneamente para usuários do cupom <span className="text-white font-bold">{couponFilter.toUpperCase()}</span>.</p>
        </section>
      </main>
    </div>
  );
