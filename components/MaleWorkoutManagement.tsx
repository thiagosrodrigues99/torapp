...
export const MaleWorkoutManagement: React.FC<MaleWorkoutManagementProps> = ({ onBack }) => {
  ...
  const fetchPlanAndExercises = async () => {
    ...
    try {
      ...
      // 2. Get existing exercises for this plan
  ...
  const saveWorkout = async () => {
    ...
    setLoading(true);
    try {
      // Clear existing exercises for this plan first (simple way to update)
        ...
        <section className="px-4 py-4">
          <div className="bg-white dark:bg-surface-dark rounded-xl p-5 shadow-sm space-y-5 border border-gray-200 dark:border-white/5">
            ...
            <button
              ...
              className="w-full bg-primary hover:bg-primary/90 text-white font-extrabold py-4 rounded-lg shadow-lg shadow-primary/20 transition-all active:scale-[0.98] uppercase tracking-wider mt-2 disabled:opacity-50 disabled:grayscale"
            >
              ADICIONAR EXERCÍCIO
            </button>
          </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                className="appearance-none text-xs font-medium bg-[#282828] text-[#f0f0f0] border-none rounded-lg h-8 pl-3 pr-8 focus:ring-1 focus:ring-primary/50 cursor-pointer"
              >
              ...
              <span className="bg-gray-200 dark:bg-surface-dark text-gray-600 dark:text-gray-400 px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap">{filteredExercises.length} Exercícios</span>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {loading && workoutExercises.length === 0 ? (
              <div className="py-10 text-center">
                ...
                <p className="text-xs text-slate-500 uppercase font-black">Carregando treino...</p>
              </div>
            ) : filteredExercises.length === 0 ? (
              <div className="py-10 text-center border-2 border-dashed border-white/5 rounded-xl">
                <p className="text-xs text-slate-500 uppercase font-black italic">Nenhum exercício na lista</p>
              </div>
            ) : (
